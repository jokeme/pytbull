    ...
    def checkNewVersionAvailable(self):
        try:
            if self.config.get('PROXY','useproxy')=='1':
                proxy_info = {
                'user' : self.config.get('PROXY','user'),
                'pass' : self.config.get('PROXY','pass'),
                'host' : self.config.get('PROXY','host'),
                'port' : int(self.config.get('PROXY','port'))
                }
            ...
            available = urllib2.urlopen('http://pytbull.googlecode.com/svn/trunk/docs/VERSION').read()
            ...
            print "***ERROR: Unable to connect to googlecode. If you use a proxy, check your configuration."
            sys.exit()
...
@                                           @   @        @                  @
        ...
        self.writeReport("<ul>")
        for module in self.modules:
            self.writeReport("""<li><a href="#%s">%s</a></li>""" % (module[1], module[0]))
        self.writeReport("""
            </ul>
